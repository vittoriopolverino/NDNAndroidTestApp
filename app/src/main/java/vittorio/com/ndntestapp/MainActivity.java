package vittorio.com.ndntestapp;

import android.app.ActivityManager;
import android.content.BroadcastReceiver;
import android.content.Context;
import android.content.Intent;
import android.content.IntentFilter;
import android.os.AsyncTask;
import android.os.Bundle;
import android.support.v7.app.AppCompatActivity;
import android.util.Log;
import android.widget.TextView;

import net.named_data.jndn.Data;
import net.named_data.jndn.Face;
import net.named_data.jndn.Interest;
import net.named_data.jndn.InterestFilter;
import net.named_data.jndn.Name;
import net.named_data.jndn.OnInterestCallback;
import net.named_data.jndn.OnRegisterFailed;
import net.named_data.jndn.security.KeyChain;
import net.named_data.jndn.security.KeyType;
import net.named_data.jndn.security.identity.IdentityManager;
import net.named_data.jndn.security.identity.MemoryIdentityStorage;
import net.named_data.jndn.security.identity.MemoryPrivateKeyStorage;
import net.named_data.jndn.security.policy.SelfVerifyPolicyManager;
import net.named_data.jndn.transport.Transport;
import net.named_data.jndn.util.Blob;

import java.io.IOException;
import java.io.OutputStream;
import java.io.PrintStream;
import java.net.InetAddress;
import java.net.NetworkInterface;
import java.net.Socket;
import java.net.SocketException;
import java.nio.ByteBuffer;
import java.util.Enumeration;

import vittorio.com.ndntestapp.NdnManager.Counter;
import vittorio.com.ndntestapp.NdnManager.Echo;
import vittorio.com.ndntestapp.Service.SocketCommunicationService;

public class MainActivity extends AppCompatActivity {

    private static final String TAG = "NDNTest";
    private TextView twServiceStatus;
    private TextView twTestLaunchedFrom;
    private TextView twTestName;
    private TextView twTestDescription;
    private TextView twRole;
    private TextView twNfdAddress;
    private TextView twNdnResponse;
    private TextView twMessageToRobot;

    private ServiceToActivity serviceReceiver;

    private String mLocalAddress;
    private String mTestLaunchedFromAddress;
    private String mTestLaunchedFromPortNumber;
    private String mTestName;
    private String mTestDescription;
    private String mRole;
    private String mNfdAddress;

    private Socket mSocket;
    private String mMessageToRobot;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        twServiceStatus = (TextView) findViewById(R.id.serviceStatus);
        twTestLaunchedFrom = (TextView) findViewById(R.id.address);
        twTestName = (TextView) findViewById(R.id.testCaseName);
        twTestDescription = (TextView) findViewById(R.id.testCaseDescription);
        twRole = (TextView) findViewById(R.id.role);
        twNfdAddress =(TextView) findViewById(R.id.nfdAddress);
        twNdnResponse = (TextView) findViewById(R.id.ndnResponse);
        twMessageToRobot = (TextView) findViewById(R.id.msgToRobot);

        Intent intent = new Intent(this, SocketCommunicationService.class);
        startService(intent);

        mLocalAddress = getIpAddress();

        if (isMyServiceRunning(SocketCommunicationService.class)) {
            twServiceStatus.setText("is running on address " + mLocalAddress);
        } else {
            twServiceStatus.setText("isn't running");
        }
        serviceReceiver = new ServiceToActivity();
        IntentFilter intentSFilter = new IntentFilter("ServiceToActivityAction");
        registerReceiver(serviceReceiver, intentSFilter);
    }

    public class ServiceToActivity extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            // get data from socket communication with robot
            Bundle notificationData = intent.getExtras();
            mTestLaunchedFromAddress = notificationData.getString("testLaunchedFrom");
            mTestLaunchedFromPortNumber = notificationData.getString("portNumber");
            mTestName = notificationData.getString("testCaseName");
            mTestDescription = notificationData.getString("testCaseDescription");
            mRole = notificationData.getString("role");
            mNfdAddress = notificationData.getString("nfdAddress");

            // open socket to communicate with robot
            mSocket = new SocketHandler().getSocket();

            // show messagge on text view component
            twTestLaunchedFrom.setText(mTestLaunchedFromAddress + ": " + mTestLaunchedFromPortNumber);
            twTestName.setText(mTestName);
            twTestDescription.setText(mTestDescription);
            twRole.setText(mRole);
            twNfdAddress.setText(mNfdAddress);

            if (mRole.equals("consumer")) {
                new Consumer().execute();
            } else {
                //new Producer().execute();
                //new NetThread().start();
                //new Producer();
                //new TestRemotePrefixRegistration().start();
            }
        }
    }

    private boolean isMyServiceRunning(Class<SocketCommunicationService> serviceClass) {
        ActivityManager manager = (ActivityManager) getSystemService(Context.ACTIVITY_SERVICE);
        for (ActivityManager.RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
            if (serviceClass.getName().equals(service.service.getClassName())) {
                return true;
            }
        }
        return false;
    }

    //CONSUMER
    private class Consumer extends AsyncTask<Void, Void, String> {

        private Face mFace;
        private Counter mCounter;

        @Override
        protected String
        doInBackground(Void... voids) {
            try {
                mFace = new Face(mNfdAddress);
                mCounter = new Counter();

                Name name = new Name("/ndn/test");
                mFace.expressInterest(name, mCounter, mCounter);

                // The main event loop.
                while (mCounter.getmCallbackCount() < 1) {
                    mFace.processEvents();
                    // We need to sleep for a few milliseconds so we don't use 100% of
                    //   the CPU.
                    Thread.sleep(1);
                }

                mFace.shutdown();
                mFace = null;

                return mCounter.getFeedback();

            } catch (Exception e) {
                return "ERROR: " + e.getMessage();
            }
        }

        @Override
        protected void
        onPostExecute(String result) {
            twNdnResponse.setText(result);
            mMessageToRobot = result;
            SocketServerReplyThread socketServerReplyThread = new SocketServerReplyThread(mSocket);
            socketServerReplyThread.run();
        }

    }

    private class SocketServerReplyThread extends Thread {

        private Socket hostThreadSocket;
        int count;

        SocketServerReplyThread(Socket socket) {
            hostThreadSocket = socket;
            //count = c;
        }

        @Override
        public void run() {
            OutputStream outputStream;
            String msgReply = "Hello from  " + mLocalAddress + " " + mMessageToRobot;

            try {
                outputStream = hostThreadSocket.getOutputStream();
                PrintStream printStream = new PrintStream(outputStream);
                printStream.print(msgReply);
                printStream.close();
                twMessageToRobot.setText(msgReply);

            } catch (IOException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
                twMessageToRobot.setText("Something wrong! " + e.toString());
            }
        }
    }


    private String getIpAddress() {
        String ip = "";
        try {
            Enumeration<NetworkInterface> enumNetworkInterfaces = NetworkInterface
                    .getNetworkInterfaces();
            while (enumNetworkInterfaces.hasMoreElements()) {
                NetworkInterface networkInterface = enumNetworkInterfaces
                        .nextElement();
                Enumeration<InetAddress> enumInetAddress = networkInterface
                        .getInetAddresses();
                while (enumInetAddress.hasMoreElements()) {
                    InetAddress inetAddress = enumInetAddress.nextElement();

                    if (inetAddress.isSiteLocalAddress()) {
                        ip += inetAddress.getHostAddress() + "\n";
                    }
                }
            }
        } catch (SocketException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
            ip += "Something Wrong! " + e.toString() + "\n";
        }
        return ip;
    }

    //PRODUCER
    private class Producerr extends AsyncTask<Void, Void, String> {

        private Face mFace;

        @Override
        protected String
        doInBackground(Void... voids) {
            try {
                mFace = new Face(mNfdAddress);

                MemoryIdentityStorage identityStorage = new MemoryIdentityStorage();
                MemoryPrivateKeyStorage privateKeyStorage = new MemoryPrivateKeyStorage();
                KeyChain keyChain = new KeyChain(new IdentityManager(identityStorage, privateKeyStorage), new SelfVerifyPolicyManager(identityStorage));
                keyChain.setFace(mFace);

                // Initialize the storage.
                Name keyName = new Name("/testname/DSK-123");
                Name certificateName = keyName.getSubName(0, keyName.size() - 1).append("KEY").append(keyName.get(-1)).append("ID-CERT").append("0");
                identityStorage.addKey(keyName, KeyType.RSA, new Blob(DEFAULT_RSA_PUBLIC_KEY_DER, false));
                privateKeyStorage.setKeyPairForKeyName
                        (keyName, KeyType.RSA, DEFAULT_RSA_PUBLIC_KEY_DER, DEFAULT_RSA_PRIVATE_KEY_DER);

                mFace.setCommandSigningInfo(keyChain, certificateName);

                Echo echo = new Echo(keyChain, certificateName);
                Name prefix = new Name("/ndn/test");
                System.out.println("Register prefix  " + prefix.toUri());
                try {
                    mFace.registerPrefix(prefix, echo, echo);
                } catch (final java.net.SocketTimeoutException e) {
                    System.out.println("socket timeout exception");
                }
                // The main event loop.
                // Wait to receive one interest for the prefix.
                while (echo.getResponseCount() < 1) {
                    mFace.processEvents();
                    // We need to sleep for a few milliseconds so we don't use 100% of
                    //   the CPU.
                    Thread.sleep(500);
                }

                return echo.getFeedback();

            } catch (Exception e) {
                return "ERROR: " + e.getMessage();
            }
        }

        @Override
        protected void
        onPostExecute(String result) {
            twNdnResponse.setText(result);
            mMessageToRobot = result;
            SocketServerReplyThread socketServerReplyThread = new SocketServerReplyThread(mSocket);
            socketServerReplyThread.run();
        }

        private ByteBuffer toBuffer(int[] array) {
            ByteBuffer result = ByteBuffer.allocate(array.length);
            for (int anArray : array) result.put((byte) (anArray & 0xff));

            result.flip();
            return result;
        }

        private final ByteBuffer DEFAULT_RSA_PUBLIC_KEY_DER = toBuffer(new int[]{
                
        });
        // Java uses an unencrypted PKCS #8 PrivateKeyInfo, not a PKCS #1 RSAPrivateKey.
        private final ByteBuffer DEFAULT_RSA_PRIVATE_KEY_DER = toBuffer(new int[]{
                
        });

    }

    private static ByteBuffer toBuffer(int[] array) {
        ByteBuffer result = ByteBuffer.allocate(array.length);
        for (int anArray : array) result.put((byte) (anArray & 0xff));

        result.flip();
        return result;
    }

    private static final ByteBuffer DEFAULT_RSA_PUBLIC_KEY_DER = toBuffer(new int[] {  });

    // Java uses an unencrypted PKCS #8 PrivateKeyInfo, not a PKCS #1
    // RSAPrivateKey.
    private static final ByteBuffer DEFAULT_RSA_PRIVATE_KEY_DER = toBuffer(new int[] {  });

    private static class Echos implements OnInterestCallback, OnRegisterFailed {
        private Echos(KeyChain keyChain) {
            keyChain_ = keyChain;
        }

        public void onInterest(Name prefix, Interest interest, Face face, long interestFilterId,
                               InterestFilter filter) {
            ++responseCount_;

            // Make and sign a Data packet.
            String message ="Got interest with name " + interest.getName().toUri();

            Log.i(TAG, message);
            Data data = new Data(interest.getName());
            String content = "200";
            data.setContent(new Blob(content));
            // setTimestampMilliseconds is needed for BinaryXml compatibility.
            data.getMetaInfo().setTimestampMilliseconds(System.currentTimeMillis());


            Log.i(TAG, "Sent content " + content);
            try {
                face.putData(data);
            } catch (IOException ex) {
                Log.i(TAG, "Echo: IOException in sending data " + ex.getMessage());
            }
        }

        public void onRegisterFailed(Name prefix) {
            ++responseCount_;
            Log.i(TAG,"Register failed for prefix " + prefix.toUri());
        }

        KeyChain keyChain_;
        Name certificateName_;
        int responseCount_ = 0;
    }

    private class NetThread extends Thread {

        @Override
        public void run() {
            try {

                Name Prefix1= new Name("/ndn/test");
                Boolean isDone = false;
                final KeyChain keyChain = new KeyChain();

                final Face face = new Face(mNfdAddress);

                face.setCommandSigningInfo(keyChain, keyChain.getDefaultCertificateName());
                /*face.registerPrefix(Prefix1, new OnInterest() {
                    public void onInterest(Name prefix, Interest interest, InterestFilter filter, long interestFilterId) {
                        Name interestName = interest.getName();
                        Data data = new Data(interestName);
                        data.setContent(new Blob("PUB1->DATA-01.".getBytes()));
                        int hourMilliseconds = 600 * 1000;
                        data.getMetaInfo().setFreshnessPeriod(hourMilliseconds);
                        try {
                            keyChain.sign(data, keyChain.getDefaultCertificateName());
                        } catch (SecurityException pE) {
                            pE.printStackTrace();
                        }
                        try {
                            face.send(data.getContent().buf());
                            face.putData(data);
                        } catch (IOException pE) {
                            pE.printStackTrace();
                        }
                        System.out.println("Replied to Interest:" + interestName.toUri());
                        System.out.println("Replied with Data name:" + interestName.toUri());

                    }
                });*/
                System.out.println("Registering listening prefix1 : " + Prefix1.toUri());

                while (!isDone) {
                    face.processEvents();
                    Thread.sleep(5);
                }



               /* Face face = new Face(mNfdAddress);

                KeyChain keyChain = buildTestKeyChain();

                face.setCommandSigningInfo(keyChain, keyChain.getDefaultCertificateName());

                //face.setCommandSigningInfo(keyChain, certificateName);

                Echos echo = new Echos(keyChain);
                Name prefix = new Name("/ndn/test");
                Log.i(TAG, "Register prefix  " + prefix.toUri());
                face.registerPrefix(prefix, echo, echo);

                // The main event loop.
                // Wait to receive one interest for the prefix.
                while (echo.responseCount_ < 1) {
                    face.processEvents();

                    // We need to sleep for a few milliseconds so we don't use 100% of
                    //   the CPU.
                    Thread.sleep(5);
                }*/
            }
            catch (Exception e) {
                System.out.println("exception: " + e.getMessage());
            }
        }

        public KeyChain buildTestKeyChain() throws net.named_data.jndn.security.SecurityException {
            MemoryIdentityStorage identityStorage = new MemoryIdentityStorage();
            MemoryPrivateKeyStorage privateKeyStorage = new MemoryPrivateKeyStorage();
            IdentityManager identityManager = new IdentityManager(identityStorage, privateKeyStorage);
            KeyChain keyChain = new KeyChain(identityManager);
            try {
                keyChain.getDefaultCertificateName();
            } catch (net.named_data.jndn.security.SecurityException e) {
                keyChain.createIdentity(new Name("/test/identity"));
                keyChain.getIdentityManager().setDefaultIdentity(new Name("/test/identity"));
            }
            return keyChain;
        }
    }

}

